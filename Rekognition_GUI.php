...
class Rekognition_GUI{
  ...
  public function DrawRectangle($x1, $y1, $x2, $y2, $strength = 5) {
    $canvas = imagecreatefromstring($this->raw_image_);
  ...
  public function DrawObject($obj, $strength = 5) {
    $this->DrawRectangle($obj->GetX(), $obj->GetY(), $obj->GetX() + $obj->GetWidth(), $obj->GetY() + $obj->GetHeight());
  }
  ...
  public function DrawObjects($objs, $strength = 5) {
    $canvas = imagecreatefromstring($this->raw_image_);
    $color = imagecolorallocate($canvas, $this->r_, $this->g_, $this->b_);
